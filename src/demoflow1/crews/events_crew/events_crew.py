...
class EventsCrew:
    ...
    def events_searcher(self) -> Agent:
        return Agent(
            config=self.agents_config["events_searcher"],  # type: ignore[index]
        )

    @agent
    def events_scraper(self) -> Agent:
        return Agent(
            config=self.agents_config["events_scraper"],  # type: ignore[index]
        )

    @agent
    def events_compiler(self) -> Agent:
        return Agent(
            config=self.agents_config["events_compiler"],  # type: ignore[index]
        )
