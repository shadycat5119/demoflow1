...
class PeopleCrew:
    ...
    def people_searcher(self) -> Agent:
        return Agent(
            config=self.agents_config["people_searcher"],
    ...
    def people_scraper(self) -> Agent:
        return Agent(
            config=self.agents_config["people_scraper"],
            llm=lambda x: completion(model=self.gemini_model, messages=[{"role": "user", "content": x}]),
    ...
    def people_compiler(self) -> Agent:
        return Agent(
            config=self.agents_config["people_compiler"],
            llm=lambda x: completion(model=self.gemini_model, messages=[{"role": "user", "content": x}]),
            verbose=True
        )
    ...
    @task
    def people_search(self) -> Task:
        return Task(
            config=self.tasks_config["people_search"],  # type: ignore[index]
        )

    @task
    def people_scrape(self) -> Task:
        return Task(
            config=self.tasks_config["people_scrape"],  # type: ignore[index]
        )

    @task
    def people_compile(self) -> Task:
        return Task(
            config=self.tasks_config["people_compile"],  # type: ignore[index]
        )

    @crew
    def crew(self) -> Crew:
        """Creates the People Crew"""
